...
export function IntermediateMatchRulePanel({
  categories,
  zoneCategories,
  pairs,
  config,
  ...
  const filterCategories = showZoneMatches ? zoneCategories : categories;
  ...
  const rules = useMemo(() => {
    ...
    const intermediateMatches = config.categories.flatMap((categoryConfig) =>
    ...
    const zoneMatches = showZoneMatches
      ...
      : [];
    const matches = [...zoneMatches, ...intermediateMatches];
  ...
  function toggleIncludeZones(on: boolean) {
    setShowZoneMatches(on);
    if (!on) return;
    setSelectedIds((current) => {
      ...
      <CardContent className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <label className="flex cursor-pointer items-center gap-2 rounded-md border px-2.5 py-1.5 text-sm">
            <Checkbox
              ...
              onCheckedChange={(value) => toggleIncludeZones(value === true)}
        ...
        {categories.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Ninguna categoría tiene fase intermedia.
          </p>
